...
--
-- RECENCY GATES (2026-07-24, after the Kissflow false-positive): static
-- signals no longer score on their own. `plan=poc` and the multi-cluster
-- rules only fire when the tenant shows life (avg QPS >= 0.001) or is a
-- fresh signup (<=48h). A tenant with zero QPS, no fresh signup and no error
-- is force-demoted to P2 regardless of its static score, so dormant legacy
-- PoC tenants (e.g. 3 starter clusters from 2023-2025) stop occupying
-- P1 slots. The moment a dormant tenant shows QPS again it re-enters
-- automatically — that is the true "win-back reopened" trigger.
--
...
WITH base AS (
    SELECT
      ...
      CASE
        -- DORMANCY GATE: zero QPS + not a fresh signup + no error = no live
      -- signal. Static inventory (old clusters, lingering poc plan, card on
      -- file) must not hold a P0/P1 slot. Re-entry is automatic on any QPS.
      WHEN qps < 0.001 AND n48 = 0 AND err = 0
          THEN 'P2'
      ...
      WHEN (act + fit + urg) >= 45
          ...
          OR (cc >= 3 AND card = 0 AND cat <> 'individual' AND qps >= 0.001)
          OR err = 1
          THEN 'P1'
        ELSE 'P2'
      END AS my_priority
    FROM (
      ...
      SELECT
        ...
        -- RECENCY GATES: poc plan and multi-cluster only score when the tenant
        -- is alive (QPS) or fresh (<=48h). A poc plan set during a 2024 PoC and
        -- never changed is inventory, not urgency.
        LEAST(30,
          (CASE WHEN n24 = 1 AND cat = 'company_ai_core'    THEN 20 ELSE 0 END)
          + (CASE WHEN n48 = 1 AND cat = 'company_ai_related' THEN 15 ELSE 0 END)
          + (CASE WHEN pl = 'poc' AND (qps >= 0.001 OR n48 = 1) THEN 15 ELSE 0 END)
          + (CASE WHEN qps >= 50  THEN 15 ELSE 0 END)
          + (CASE WHEN cc >= 3 AND card = 0 AND cat <> 'individual' AND qps >= 0.001 THEN 12 ELSE 0 END)
          ...
          FROM (
            SELECT CONCAT(
              ...
              COALESCE(NULLIF(CONCAT_WS('·',
                ...
                -- POC label only when the gated signal actually fired; a dormant
                -- poc plan shows as 旧POC so readers don't mistake it for a live one
                CASE WHEN pl = 'poc' AND (qps >= 0.001 OR n48 = 1) THEN 'POC'
                     WHEN pl = 'poc' THEN '旧POC' ELSE NULL END,
